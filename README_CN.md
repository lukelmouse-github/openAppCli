...
**通杀所有 Android 应用** —— 包括微信、银行类应用、WebView、Flutter 以及安全保护的应用。当 UI 检查被阻止时，AI 自动降级为截图 + 坐标点击。没有任何应用能逃脱。
...
OpenAppCli 通过双层策略支持**所有** Android 应用：

| 应用类型 | 方式 | 示例 |
|----------|------|------|
| 标准应用 | UI 检查（语义选择器） | 大多数应用 |
| 安全保护应用 | 截图 + tap 坐标 | 微信、银行类应用 |
| WebView / Flutter | 截图 + tap 坐标 | 混合应用 |
