...
> **AI 自主探索任何应用，自动封装能力为 CLI，沉淀为可复用资产。**

**通杀所有应用** —— 包括微信、银行类应用、WebView、Flutter 以及安全保护的应用。当 UI 检查被阻止时，AI 自动降级为截图 + 坐标点击。没有任何应用能逃脱。

OpenAppCli 让 AI 代理能够自主发现、自动化并封装任何应用的功能为可复用的命令行工具 —— 无需修改 App，无需特殊权限，无需手写脚本。

## 为什么需要 OpenAppCli？

**问题**：每个应用都是一个能力孤岛。要自动化它，你需要手动编写脚本、维护它们，然后对每个应用重复这个过程。
...
### 环境要求
- Node.js 18+
- 设备已开启 USB 调试（目前支持 Android，iOS 计划中）
...
OpenAppCli 通过双层策略支持**任何应用**：

| 应用类型 | 方式 | 示例 |
|----------|------|------|
| UI 可访问的应用 | UI 检查（语义选择器） | 大多数应用 |
| 安全保护 / UI 被阻止的应用 | 截图 + tap 坐标 | 微信、银行类应用、WebView、Flutter |

**没有应用能逃脱。** 当应用阻止 UI 检查时，AI 自动降级为视觉分析 + 归一化坐标。
...
**核心洞察**：AI 不只是使用应用 —— 它**把应用封装成 CLI 工具**。每次交互都沉淀为可复用资产，构建不断增长的应用自动化库。
